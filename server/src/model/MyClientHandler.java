...
public class MyClientHandler extends AbstractClientHandler {
	...
	private void CheckGenerate(BufferedReader inFromClient, ObjectOutputStream outToClient) {
		...
		try {
			...
			outToClient.writeObject("What is the number of floors");
			...
			outToClient.writeObject("What is the number of rows");
			...
			outToClient.writeObject("What is the number of columns");
	...
	private void checkSolution(BufferedReader inFromClient, ObjectOutputStream outToClient) {
		...
		try {
			...
			while(solution != null)
			{
				int first = solution.indexOf("{");
				int last = solution.indexOf("}");
				
				if(last < first + 1)
					break;
				
				String line1 = solution.substring(first+1, last);
				
				solution = solution.substring(last+1, solution.length());
				
				System.out.println(line1);
				outToClient.writeObject(line1);
